...
from openvino.tools.mo.ops.fakequantize import FakeQuantize, FakeConvertFP
...
class FakeQuantizePropagation(BackReplacementPattern):
    ...
    jump_single_branch_ops = ['ReduceMax', 'MaxPool', 'Reshape', 'Flatten', 'Squeeze', 'Unsqueeze', 'Interpolate',
                              ...
                              'Gather']
    remove_duplication_ops = ['FakeConvertFP', 'Parameter']
    ...
    def delete_fq_non_quantizable_node_precision(self, graph):
        ...
        node_int_fq = []
        fq_queue = deque(sorted(graph.get_op_nodes(type='FakeConvertFP'), key=lambda x: x.name))
        while fq_queue:
            fq = fq_queue.popleft()
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        # Iterate over FakeConvertFP operations and push them on top while it's possible
        while fq_queue:
            # Get FakeConvertFP from queue and it's input node
            fq = fq_queue.popleft()

            # In case if we already touched this FakeConvertFP it could be disconnected from the main graph
            ...
            if m_op \
                ...
                elif isinstance(new_fq, Node) and new_fq.type == 'FakeConvertFP':
...
class RemoveFakeQuantize:
    ...
    def find_fq_nodes_to_cut(self, node):
        def parse_node_relatives(node, is_parents):
            ...
            for relative_port in relatives_ports:
                ...
                if relative.type == 'FakeConvertFP':
                    if is_parents:
    ...
    def optimize_for_gp_hw(self, graph, target_device):
        ...
        check_is_inputs_fq = lambda node: all([op.type == 'FakeConvertFP' for op in node])
    ...
    def undo_weights_rescaling(conv_node):
        ...
        if weights_node.type == 'FakeConvertFP':
...
class FakeQuantizeNameSwapper(BackReplacementPattern):
    ...
    def rename_fqs_in_the_end(self, graph: Graph):
        def change_names(_, match):
            ...
            if len(input_node_outputs) > 1 and all([op.type == 'FakeConvertFP' for op in input_node_outputs]):
...
def create_fake_quantize_node(graph: Graph, name, data_type=np.float32, **kwargs):
    fq = FakeConvertFP(graph, {'name': name,
                              'stop_value_propagation': True, **kwargs}).create_node()

    input_scale = Const(graph, {'value': np.array(0.0, dtype=data_type)}).create_node()
    input_scale.out_port(0).connect(fq.in_port(1))
    input_scale.infer(input_scale)

    input_shift = Const(graph, {'value': np.array(0.0, dtype=data_type)}).create_node()
    input_shift.out_port(0).connect(fq.in_port(2))
    input_shift.infer(input_shift)
...
def insert_fake_quantize(graph, node, ports=None, names=None, fq_types=None, hw_config=None, input_priority_types=[]):
    ...
    for idx, port in node.in_ports().items():
        ...
        # Create FakeConvertFP operations
        ...
        # Insert FakeConvertFP after input
        if node.type == 'Result':
...
def insert_output_fake_quantize(graph, node, hw_config=None, ignored_params=None):
    ...
    for out_port_id, port in node.out_ports().items():
        ...
        for next_port_id, next_port in enumerate(next_ports):
            ...
            if ignored_params is not None and next_node.type != 'FakeConvertFP' \
