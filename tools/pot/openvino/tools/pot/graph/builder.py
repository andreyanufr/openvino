...
def make_copy_fake_quantize(nodes, edges, fq):
    weights, scales, shift = get_node_inputs(fq)
    ...
    nodes.extend([
        ...
        (shift.name, shift.type, {'value': shift.value.copy()})
    ])

    edges.extend([
        (weights.name, fq.name, {'out': 0, 'in': 0}),
        (scales.name, fq.name, {'out': 0, 'in': 1}),
        (shift.name, fq.name, {'out': 0, 'in': 2})
    ])
    return fq.name
...
def build_graph_for_node(model, input_name, input_shape, node, remove_bias=False, remove_fake_quantize=False):
    ...
    if parent_nodes[1].type == 'FakeConvertFP' and not remove_fake_quantize:
        fq = parent_nodes[1]
    ...
    if not remove_bias:
        if parent_nodes[2].type == 'FakeConvertFP' and not remove_fake_quantize:
            fq = parent_nodes[1]
            fq_name = make_copy_fake_quantize(nodes, edges, fq)
    ...
    weights_node = get_node_input(weights_node, 0) \
        if weights_node.type == 'FakeConvertFP' else weights_node
