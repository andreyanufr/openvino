...
algorithms = [
    {
        ...
        "params": {
            ...
            "alpha": 0.95, # you can try other values 0.5 or 0.15
        },
    },
    {
...
#Step 6 : Evaluate the original model. Print the results.
...
compressed_model_paths = save_model(model=compressed_model, save_path=MODEL_DIR, model_name="int8_last_sq_alpha_0.5_quantized_bert_mrpc")
...
# Step 9 (Optional): Evaluate the compressed model and print the results.
...
model_config = Dict({"model_name": "bert_mrpc", "model": compressed_model_xml,
                     "weights": compressed_model_xml.replace('.xml', '.bin')})

model = load_model(model_config=model_config)
int_results = pipeline.evaluate(model=model)
