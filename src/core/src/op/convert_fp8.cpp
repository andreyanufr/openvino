...
BWDCMP_RTTI_DEFINITION(op::v10::FakeConvertFP);

op::v10::FakeConvertFP::FakeConvertFP() : Op(), m_destination_type("HF8"), m_apply_scale(true) {}

//! [op:ctor]
op::v10::FakeConvertFP::FakeConvertFP(const ov::Output<ov::Node>& arg,
...
const std::vector<std::string> op::v10::FakeConvertFP::m_valid_types({"HF8", "BF8"});

//! [op:validate]
void op::v10::FakeConvertFP::validate_and_infer_types() {
    set_output_type(0, get_input_element_type(0), get_input_partial_shape(0));
}
//! [op:validate]

//! [op:copy]
std::shared_ptr<ov::Node> op::v10::FakeConvertFP::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    OPENVINO_ASSERT(new_args.size() == 3, "Incorrect number of new arguments");

    return std::make_shared<FakeConvertFP>(new_args.at(0), new_args.at(1), new_args.at(2), m_destination_type, m_apply_scale);
}
//! [op:copy]

//! [op:visit_attributes]
bool op::v10::FakeConvertFP::visit_attributes(ov::AttributeVisitor& visitor) {
    validate();
...
void op::v10::FakeConvertFP::validate() const {
...
bool op::v10::FakeConvertFP::has_evaluate() const {
    return true;
}
...
void convertfp16_hf8_bias7(const T* arg, T* out, size_t count, int exp_bits = 5, int mbits = 9, bool use_clamp = true) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        short exp_h =
            (short)((h.u & 0x7C00) >> 10) - 15;  /// 0111110000000000 -> 0000000000011111 - 15 - biased exponent for fp16
        short sign_h = (h.u & 0x8000);           /// & 1 00000 0000000000
        short mantissa_h = (h.u & 0x03FF);       /// & 0 00000 1111111111
        ///(h.u && 0111111111111111) < 0 10010 1110000000 (19326) - ????
        unsigned short can_round = ((h.u & 0x7FFF) < 0b100111110000000) ? 1 : 0;
...
//! [op:evaluate]
bool op::v10::FakeConvertFP::evaluate(ov::TensorVector& outputs, const ov::TensorVector& inputs) const {
